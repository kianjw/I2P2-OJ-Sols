...
static int count = 0; //for codegen
int main() {
...
void semantic_check(AST *now) {
	...
	if (now->kind == ASSIGN) {
		...
		if (tmp->kind != IDENTIFIER)
			err("Lvalue is required as left operand of assign ment.");
	}
	...
	if(now->kind == PREINC || now->kind == PREDEC || now->kind == POSTINC || now->kind == POSTDEC ){
		AST *temp = now->mid;
		while (temp->kind == LPAR) temp = temp->mid;
		if (temp->kind != IDENTIFIER)
			err("Lvalue is required as INC/DNCof assignment.");	
	}
...
void codegen(AST *root) {
	...
	//for the stack, if we dont use, remove it
	 int idx;
	 AST*tmp = NULL; //for tracing parenthese
	if(root == NULL) return;
	switch(root->kind){
		case IDENTIFIER:
			idx = (root->val - 'x')*4; //for calculating 0, 4, 8
			printf("load r%d [%d]\n", count, idx);
			count++;
			return;
		case CONSTANT:
		...
		case ASSIGN:
			codegen(root->rhs);
			//codegen(root->lhs);
			tmp = root->lhs; //check whether lhs is identify
			while(tmp->kind != IDENTIFIER) tmp = tmp->mid;
			idx = (tmp->val - 'x')*4; //check x
			printf("store [%d] r%d\n", idx, count-1); 
			return;
		...
		case ADD:
			codegen(root->lhs);
			codegen(root->rhs);
			count-=2;//check the recursion, 2 steps minus 2
		...
		case SUB:
			codegen(root->lhs);
			codegen(root->rhs);
			count-=2; //after recursive and back to ADD, we remove the stack of these 2 above
		...
		case PREINC:
			...
			tmp = root->mid; //check whether lhs is identify
			while(tmp->kind != IDENTIFIER) tmp = tmp->mid;
			idx = (tmp->val - 'x')*4; //check x
			printf("store [%d] r%d\n", idx, count);
			count++;
